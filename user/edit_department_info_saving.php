...
else if( mysqli_num_rows($result) < 1)
{
	 ...
	 $insert_sql_statement="UPDATE `department_info` SET `location`='$location',`department_name`='$department_name',`section_name`='$section_name',`contact_person_name`='$contact_person_name',`contact_no`='$contact_no',`email`='$email',`recording_person_id`='$user_id',`recording_person_name`='$user_name',`recording_time`=NOW() WHERE `id`= '$id'";
